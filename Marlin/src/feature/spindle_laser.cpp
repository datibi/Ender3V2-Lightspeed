...
void SpindleLaser::init() {
  ...
  void SpindleLaser::_set_ocr(const uint8_t ocr) {
    #if ENABLED(HAL_CAN_SET_PWM_FREQ) && SPINDLE_LASER_FREQUENCY
      hal.set_pwm_frequency(pin_t(SPINDLE_LASER_PWM_PIN), TERN(MARLIN_DEV_MODE, frequency, gcode.LaserMode?SPINDLE_LASER_FREQUENCY:TEMP_TIMER_FREQUENCY));
    #endif
    hal.set_pwm_duty(pin_t(SPINDLE_LASER_PWM_PIN), gcode.LaserMode? ocr ^ SPINDLE_LASER_PWM_OFF:0 ^ SPINDLE_LASER_PWM_OFF);
  }
